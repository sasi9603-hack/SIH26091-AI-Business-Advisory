...
interface FinancialCalculatorProps {
  initialFinancials: FinancialBreakdown;
...
export const FinancialCalculator: React.FC<FinancialCalculatorProps> = ({
  ...
  const loanPrincipal = initialFinancials.loanPrincipal;
  ...
  const baseFixed = initialFinancials.fixedMonthlyCosts - initialFinancials.monthlyEmi;
  const dynamicTotalFixed = baseFixed + dynamicEmi;
  const dynamicBreakEven = Math.round(dynamicTotalFixed / (initialFinancials.grossMarginPercentage / 100));
    ...
    <div className="bg-white rounded-xl shadow-sbi border border-sbi-border p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-purple-50 to-indigo-50/50 p-4 rounded-xl border border-purple-200">
          ...
          <span className="text-2xl font-black text-sbi-navy block mt-1">₹{initialFinancials.totalProjectCost.toLocaleString('en-IN')}</span>
          <span className="text-[11px] text-slate-500 block mt-0.5">
            Machinery (₹{initialFinancials.machineryAndEquipment.toLocaleString('en-IN')}) + Setup + Buffer
          </span>
        </div>
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50/50 p-4 rounded-xl border border-emerald-200">
          ...
          <span className="text-2xl font-black text-emerald-700 block mt-1">₹{initialFinancials.subsidyAmount.toLocaleString('en-IN')}</span>
          <span className="text-[11px] text-emerald-600 font-semibold block mt-0.5">
            {initialFinancials.subsidyPercentage}% Margin Money Grant
          </span>
        </div>
        ...
        <div className="bg-gradient-to-br from-sky-50 to-blue-50/50 p-4 rounded-xl border border-sky-200">
          ...
          <span className="text-2xl font-black text-sbi-navy block mt-1">₹{loanPrincipal.toLocaleString('en-IN')}</span>
          <span className="text-[11px] text-slate-500 block mt-0.5">
            After Beneficiary Equity (₹{initialFinancials.beneficiaryContributionAmt.toLocaleString('en-IN')})
          </span>
        </div>
      </div>
        ...
        <div className="lg:col-span-6 bg-slate-50 p-5 rounded-xl border border-slate-200 space-y-5">
          ...
          <div className="pt-2 border-t border-slate-200 space-y-2 text-xs">
            ...
            <div className="flex h-3 rounded-full overflow-hidden shadow-inner">
              <div
                style={{ width: `${(initialFinancials.machineryAndEquipment / initialFinancials.totalProjectCost) * 100}%` }}
                className="bg-sbi-indigo"
                title="Machinery & CapEx"
              />
              <div
                style={{ width: `${(initialFinancials.setupAndLicensing / initialFinancials.totalProjectCost) * 100}%` }}
                className="bg-sbi-blue"
                title="Shop Setup"
              />
              <div
                style={{ width: `${(initialFinancials.workingCapitalBuffer / initialFinancials.totalProjectCost) * 100}%` }}
                className="bg-emerald-500"
                title="Working Capital Buffer"
              />
            </div>
            <div className="flex flex-wrap items-center gap-3 text-[10px] text-slate-600">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-sbi-indigo inline-block"></span> Machinery: ₹{initialFinancials.machineryAndEquipment.toLocaleString('en-IN')}</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-sbi-blue inline-block"></span> Setup: ₹{initialFinancials.setupAndLicensing.toLocaleString('en-IN')}</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-emerald-500 inline-block"></span> Buffer: ₹{initialFinancials.workingCapitalBuffer.toLocaleString('en-IN')}</span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-6 space-y-4">
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm space-y-3">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              ...
              <strong className="text-slate-900">₹{dynamicBreakEven.toLocaleString('en-IN')}</strong> in monthly billings at a {initialFinancials.grossMarginPercentage}% gross margin.
            </p>
